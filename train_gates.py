"""
train_gates.py — Train with gates
"""
import os
import numpy as np
import torch
...
def train():
    device = get_device()
    print("=" * 55)
    print("  BioDrone-RL  Gate Navigation Training")
    print("=" * 55)
    
    env = GateTunnelEnv()
    
    # Load pretrained simple model if exists
    ...
    for ep in range(1, EPISODES + 1):
        ...
        if ep % PRINT_EVERY == 0:
            ...
            print(f"  Ep {ep:4d} | Reward: {total_reward:7.1f} | "
                  f"Avg100: {avg:7.1f} | Gates: {gates} | Steps: {len(rewards)}")
